## **AI Agent Marketplace Architecture Overview**
...
- Marketplace: “Are you allowed to ask for this?”
- Product Control Plane: “What does this request mean?”
- Infra Control Plane: “How do we make it happen?”

# Discovery

As we evolve through researching deeper into certain architectural decisions this document will evolve.

Some things which are still up in the air are.

## Long Lived Agent Run Times vs Ephemeral Scheduled Jobs

- Isolation (security, dependency conflicts, trust)
  - Secrets isolation becomes brittle
  - dependencies
    - python version
    - libc / OS dependencies
    - native wheels (numpy, torch, tokenizers)
    - CUDA / CPU variants
    - LangChain version
    - Transitive dependency resolution
    - Tooling binaries (ffmpeg, pandoc, etc.)
- Operational simplicity (builds, images, scheduling)
- Economic efficiency (cold starts, reuse, density)

> You can only optimize two at once. That’s not a slogan — it’s a constraint.

### Logging/Monitoring/Reporting

1. Publish phase (Build-time)
   “Turn source code into a runnable artifact”

- Input: source code (repo, zip, tarball)
- Output: immutable image (digest)
- Failure surface: Dockerfile, dependencies, build scripts
- Audience for logs: publisher (developer)

1. Run phase (Runtime)
   “Execute a previously-built artifact with inputs”

- Input: image digest + runtime config
- Output: execution result + logs
- Failure surface: app logic, environment vars, network, model calls
